...
class LstmPolicy3(object):
    ...
    def __init__(self, X, sess, nact, ac_space, nbatch, nsteps, nlstm=32, reuse=False, name="model"):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            l3 = tf.layers.dense(inputs=l2, units=64, activation=tf.nn.tanh, name="l3")
            ...
            l7 = tf.layers.dense(inputs=l6, units=32, activation=tf.nn.tanh, name="l7")
...
class LSTM_GR_Viewer(object):
    def __init__(self, X, sess, nact, #nact=image_size [64]
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            S = tf.placeholder(tf.float32, [nenv, nlstm * 2], name="S")
